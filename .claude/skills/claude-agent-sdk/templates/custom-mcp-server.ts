...
const databaseServer = createSdkMcpServer({
  ...
  tools: [
    ...
    tool(
      ...
      async (args) => {
        try {
          // Safe implementation: Strict character allowlist validation
          // Only allow numbers, basic arithmetic operators, parentheses, dots, and whitespace
          if (!/^[0-9+\-*/().\s]+$/.test(args.expression)) {
            throw new Error("Security Error: Expression contains invalid characters. Only numbers and basic operators are allowed.");
          }

          // safe to execute after strict validation
